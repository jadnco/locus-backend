...
class Followers extends Model {
  ...
  static add(id: string, req: Object, res: Object): void {
    ...
    Followers.findOneAndUpdate({ user: id },
      ...
      .then(() => {
        ...
        return User.findByIdAndUpdate(id, { $inc: { followersCount: 1 } });
      })
  ...
  static isFollowing(user: string, follower: string, res: Object): void {
    Followers.findOne({ user })
      .then(followers => {

        // Follower has been found
        if (followers.followers.indexOf(follower) > -1) {
